...
**What scalar the policy-optimization step uses (important):** In `financial_triage_training.ipynb`, the `reward_func` that TRL’s trainer calls does **not** use the full dense return from the observation, and does **not** call **`grade_episode`**. Each generated completion runs **one** `env.step` on the parsed action, then maps that transition to a hand-written score in **[-1, 1]**: a base, a lift if checking stays non-negative, a lift if `daily_summary` signals on-time pay, a cut if the text suggests an informal loan—cheap to score online. **By contrast,** the **bar chart** averages **full** episodes via **`get_episode_score()` → `grade_episode`** (**a score between 0 and 1**; [Episode grade](#episode-grade)). **Policy phase = one-step proxy; bar heights = full-run episode grader.**
